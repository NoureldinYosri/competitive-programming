...
#define volume xx.x
#define num xx.y
#define aftermath yy.x
#define confidence yy.y
...
int main(){
	int n; vi r;
	scanf("%d",&n);
	loop(i,n) scanf("%I64d %I64d %I64d",&C[i].volume,&C[i].aftermath,&C[i].confidence),C[i].num = i + 1;
	loop(i,n){
		if(C[i].confidence < 0) continue;
		r.pb(C[i].num);
		ll scream = 0,vol = C[i].volume;
		int j = i + 1;
		while(j < n){
			while(j < n && C[j].confidence < 0) j++;
			C[j].confidence -= vol + scream; vol -= vol > 0;
			if(C[j].confidence < 0) scream += C[j].aftermath;
			j++;	
		}		
	}
	cout << sz(r) << endl;
	loop(i,sz(r)) cout << r[i] << ((i == sz(r) - 1) ? "\n" : " ");
	return 0;
}
