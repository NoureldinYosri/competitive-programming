...
int main(){
	int n,s,p,q; char ord[3];
	scanf("%d %d",&n,&s);
	loop(i,n){
		scanf("%s %d %d",ord,&p,&q);
		if(ord[0] == 'S'){
			if(S.find(p) == S.end()) S[p] = 0;
			S[p] += q;
		}
		else{
			if(B.find(p) == B.end()) B[p] = 0;
			B[p] += q;
		}
	}
	int i = 0; vector<pair<int,int > > V;
	for(auto it = S.begin();it != S.end() && i < s;i++,it++) V.pb(mp(it->xx,it->yy));
	reverse(V.begin(),V.end());
	for(auto p : V) printf("S %d %d\n",p.xx,p.yy);
	i = 0;
	for(auto it = B.rbegin();it != B.rend() && i < s;i++,it++) printf("B %d %d\n",it->xx,it->yy);	
	return 0;
}
