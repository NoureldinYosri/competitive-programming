...
ll solve(int target,int pos){
	...
	ll a = solve(target + 1,lst[target]);
	//if(pos <= fst[target])

	a += get_cost(pos,fst[target],target) + get_cost(fst[target],lst[target],target) + 1;
	/*else {
		a += get_cost(fst[target],pos,target);
		a += get_cost(fst[target],pos,target + 1);
		a += get_cost(pos,lst[target],target);
	}
	*/
	ll b = solve(target + 1,fst[target]);
	//if(pos >= lst[target])
		b += get_cost(lst[target],pos,target) + get_cost(fst[target],lst[target],target) + 1;
	//else {
